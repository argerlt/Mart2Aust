function M2A_export_ang(ebsd,fName,varargin)
...
% Write phase info
for phaseId = fliplr(1:size(ebsd.CSList,2))
    cs = ebsd.CSList{phaseId};
    if cs ~= "notIndexed"
        fprintf(filePh,'# %s %.0f\n','Phase',ebsd.phaseMap(phaseId)+1);
...
% if OR data exists, add it to the ebsd file
try
    OR = join(string(ebsd.opt.OR),"  ");
    HW = string(ebsd.opt.HW);
catch
    OR = ''
    HW = ''
end
